"""
model_building.py — Model Construction using the Strategy Design Pattern
========================================================================

PURPOSE:
    Builds and trains a scikit-learn model pipeline on the training data.
    Uses the **Strategy pattern** to make the model type easily swappable.
...
CONTEXT CLASS:
    ModelBuilder                — Accepts any ModelBuildingStrategy and calls build_and_train_model().

NOTE:
    This file is the "clean" reference implementation. The actual ZenML step
    (steps/model_building_step.py) builds its own pipeline with ColumnTransformer
    to handle both categorical and numerical features, plus MLflow logging.

USED BY:
    Referenced as a design pattern example. The actual training step uses its own inline pipeline.
"""
