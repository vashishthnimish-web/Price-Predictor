"""
data_splitter.py — Train/Test Splitting using the Strategy Design Pattern
=========================================================================

PURPOSE:
    Splits a cleaned DataFrame into training and testing sets (features + target).
    Uses the **Strategy pattern** so different split methods (simple, k-fold, time-based, etc.)
    can be added without modifying the existing code.

AVAILABLE STRATEGIES:
    1. SimpleTrainTestSplitStrategy — 80/20 random split using sklearn's train_test_split.

CONTEXT CLASS:
    DataSplitter                    — Accepts any DataSplittingStrategy and calls split_data().

USED BY:
    steps/data_splitter_step.py  →  creates DataSplitter(SimpleTrainTestSplitStrategy())
"""
