"""
model_building_step.py — ZenML Step: Train the Model with MLflow Tracking
==========================================================================
...
PIPELINE POSITION:  Step 6 of 7 in the training pipeline.
INPUT:              X_train (pd.DataFrame), y_train (pd.Series)
OUTPUT:             Trained sklearn Pipeline (saved as ZenML artifact "sklearn_pipeline")

KEY DETAILS:
    - Uses ColumnTransformer to handle both numeric (SimpleImputer) and categorical
      (SimpleImputer + OneHotEncoder) features automatically.
    - MLflow autologging captures: model parameters, training metrics, and the model artifact.
    - The trained pipeline is registered as a ZenML Model artifact for deployment.
"""
